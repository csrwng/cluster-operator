...
func NewMasterController(machineInformer informers.MachineInformer, kubeClient kubeclientset.Interface, clusteroperatorClient clusteroperatorclientset.Interface) *MasterController {
	...
	if kubeClient != nil && kubeClient.CoreV1().RESTClient().GetRateLimiter() != nil {
		metrics.RegisterMetricAndTrackRateLimiterUsage("clusteroperator_master_machine_controller", kubeClient.CoreV1().RESTClient().GetRateLimiter())
	}

	c := &MasterController{
		client: clusteroperatorClient,
		queue:  workqueue.NewNamedRateLimitingQueue(workqueue.DefaultControllerRateLimiter(), "masterMachine"),
	}
...
func (c *MasterController) Run(workers int, stopCh <-chan struct{}) {
	...
	glog.Infof("Starting master machine controller")
	defer glog.Infof("Shutting down master machine controller")
