...
func (s *CMServer) AddFlags(fs *pflag.FlagSet, allControllers []string, disabledByDefaultControllers []string) {
	...
	fs.Int32Var(&s.ConcurrentMachineSetSyncs, "concurrent-machine-set-syncs", s.ConcurrentMachineSetSyncs, "The number of node group objects that are allowed to sync concurrently. Larger number = more responsive node groups, but more CPU (and network) load")
	fs.Int32Var(&s.ConcurrentMachineSyncs, "concurrent-machine-syncs", s.ConcurrentMachineSyncs, "The number of node objects that are allowed to sync concurrently. Larger number = more responsive nodes, but more CPU (and network) load")
	fs.Int32Var(&s.ConcurrentMasterSyncs, "concurrent-master-syncs", s.ConcurrentMasterSyncs, "The number of master node objects that are allowed to sync concurrently. Larger number = more responsive master nodes, but more CPU (and network) load")
